...
document.getElementById('totalLetters').innerText = wordLength;

function generateWord() {

    const totalWords = allWords.length;
    const randomNumber = (parseInt(Math.random() * totalWords)) % totalWords;
    generatedWord = allWords[randomNumber];
...
function buttonClicked(buttonID) {
    ...
    for (let i = 0; i < wordLength; i++) {
        if (generatedWord[i] == letter) {
            letterCheck = true;
            const diff = i - 0;
            guessedWord = guessedWord.substr(0, diff) + letter + guessedWord.substr(diff + 1);
        }
    }
    document.getElementById('guessByPlayer').innerText = guessedWord;
